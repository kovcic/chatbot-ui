...
import { MarkdownNodeParser, SimpleDirectoryReader, SimpleNodeParser, Document, ServiceContext } from 'llamaindex';
import { extractMetadata } from './lib/rag/metadata';
import { createVectorIndex } from './lib/rag/vector-index';
import { createSummaryIndex, deleteSummaryIndex, getSummaryIndex } from './lib/rag/summary-index';
...
const run = async () => {
  const file = fs.readFileSync('/Users/andrej.kovcic/dev/ds/spark-pilot/example-docs/samples2/embedded-images-tables.md');
  ...
  // const summaryIndex = await createSummaryIndex('test', documents, { serviceContext });
  // const summaryIndex = await getSummaryIndex('test');
  // deleteSummaryIndex('test')
